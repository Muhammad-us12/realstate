                            ...
                            @endif
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <form action="{{ URL::to('offers-update/'.$offer_data->id.'') }}" method="post" enctype="multipart/form-data">
                                            @csrf
                                            <div class="row mb-2">
                                                <div class="col-sm-12">
                                                    <div class="mb-3">
                                                        <label for="example-input-normal" class="form-label">Title</label>
                                                        <input type="text" id="example-input-normal"  name="title" value="{{ $offer_data->title }}" class="form-control" placeholder="Normal">
                                                        @error('title')
                                                ...
                                                <div class="col-sm-3">
                                                    <div class="mb-3">
                                                        ...
                                                        <select class="form-select select2" name="offer_category" id="example-select">
                                                            ...
                                                            <option value="{{ $cat_res->id }}" @if($offer_data->offer_category == $cat_res->id) {{ "selected" }} @endif>{{ $cat_res->category_name }}</option>
                                                                @endforeach
                                                            @endisset
                                                        </select>
                                                    </div>
                                                        ...
                                                        <select class="form-select select2" name="offer_location" id="example-select">
                                                            @isset($Location_data)
                                                                @foreach($Location_data as $locat_res)
                                                                    <option value="{{ $locat_res->id }}" @if($offer_data->offer_location == $locat_res->id) {{ "selected" }} @endif>{{ $locat_res->location_name }}</option>
                                                                @endforeach
                                                            @endisset
                                                        </select>
                                                    </div>
                                                ...
                                                <div class="col-sm-2">
                                                     <img src="{{ asset('/public/images/offers/'.$offer_data->picture.'') }}" style="width:100px; height:100px" alt="contact-img" title="contact-img" class="rounded me-3" height="48" />
                                                </div>
